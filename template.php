...
// Add Zen Tabs styles
if (theme_get_setting('base_theme_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'base_theme') .'/assets/css/tabs.css');
}
...
function base_theme_preprocess_html(&$vars) {
  ...
  if ($node && isset($node->nid)){
    ...
    if (!empty($field_hero_image)) {
        $vars['classes_array'][] = 'has-hero-image';
    }
  }

  // Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . base_theme_id_safe($role);
  }

  // blocks outside page.tpl.php
  $vars['modal'] = block_get_blocks_by_region('modal');
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = base_theme_id_safe('page-'. $path);
    $vars['classes_array'][] = base_theme_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('base_theme_ie_enabled')) {
    $base_theme_ie_enabled_versions = theme_get_setting('base_theme_ie_enabled_versions');
    if (in_array('ie8', $base_theme_ie_enabled_versions, TRUE)) {
      drupal_add_css(path_to_theme() . '/assets/css/ie8.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'IE 8', '!IE' => FALSE), 'preprocess' => FALSE));
      drupal_add_js(path_to_theme() . '/assets/js/vendor/selectivizr-min.js');
    }
    if (in_array('ie9', $base_theme_ie_enabled_versions, TRUE)) {
      drupal_add_css(path_to_theme() . '/assets/css/ie9.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'IE 9', '!IE' => FALSE), 'preprocess' => FALSE));
    }
    if (in_array('ie10', $base_theme_ie_enabled_versions, TRUE)) {
      drupal_add_css(path_to_theme() . '/assets/css/ie10.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'IE 10', '!IE' => FALSE), 'preprocess' => FALSE));
    }
  }

}

function base_theme_preprocess_page(&$vars, $hook) {

  $path = drupal_get_path_alias();

  // make nodes acessible to page
  if (arg(0) == 'node') {
  ...
  // all of the URLS are of a certain type use the page--article.tpl
  if($path == 'news' || $path == 'events' || $path == 'academic-calendar'){
...
function base_theme_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from base_theme_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function base_theme_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function base_theme_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('base_theme_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('base_theme_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('base_theme_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('base_theme_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('base_theme_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function base_theme_id_safe($string) {
 ...
 *
 */
function base_theme_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = base_theme_id_safe($element['#title']);
 ...
 */
function base_theme_preprocess_menu_local_task(&$variables) {
 ...
 */
function base_theme_menu_local_tasks(&$variables) {
  $output = '';
